...
DNS 高级 JSON 编辑器只显示 `{"dns": {...}}`，并且只能替换 `dns` 分区。可视化页面会在一次配置事务中同时保存 DNS 表单与解析策略：远程 DNS 策略写入全局 `dns.strategy` 并覆盖对应 DNS 路由规则，直连 DNS 策略写入对应规则的 `strategy`，节点域名策略写入 `route.default_domain_resolver.strategy`，其他路由字段保持不变。高级 JSON 中将内容改为 `{}` 会删除该分区，`null` 与删除不是一回事。
