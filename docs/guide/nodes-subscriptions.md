...
前置代理和落地代理从现有 Catalog 节点中选择，支持同分组或跨分组引用。实际连接顺序为“前置代理 -> 当前订阅节点 -> 落地代理”。运行时会生成隔离的 Provider 副本，持久化的订阅 `provider.json` 不会被改写；内部链路节点不会出现在 Auto 或手动节点列表中。已包含 `detour` 或协议不支持链式拨号的节点不能作为前置或落地代理。

代理链使用稳定的 `<分组ID>/<tag>` 引用。被引用节点或分组不能直接删除；如果被引用的节点在订阅更新后消失或变得不支持链式拨号，本次更新会失败并继续保留上一版有效 Provider。
