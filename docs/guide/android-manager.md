...
| 订阅 | 添加、编辑、更新、筛选、节点域名 DNS、流量信息、周期与更新历史 |
| 代理设置 | eBPF 接管、DNS、IPv6、共享网络、分应用与 Wi-Fi 策略 |
| 内核设置 | 可视化编辑 DNS 与本地分流规则，编辑并校验其他 sing-box 静态配置，查看只读运行时配置 |
...
订阅编辑页的“节点域名 DNS”可以选择内核 DNS 页面中已有的服务器标签。默认“继承默认”保持原有行为；选择标签后，模块只在运行时 Provider 副本中为该订阅节点设置 `domain_resolver`，不会改写订阅保存的节点，也不会把节点域名提前替换为 IP。

本地分流规则页按代理、直连和拦截分组，可编辑完整域名、域名后缀、关键词、正则、IP CIDR、端口、网络和协议。未覆盖的高级字段会原样保留，并可从当前分组进入高级 JSON 编辑器；远程规则集不在此页面修改。
